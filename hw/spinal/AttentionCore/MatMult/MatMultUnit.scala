package AttentionCore.MatMult
import AttentionCore.SystolicArray2D._
 ...
 *    MatMultUnit
 *
 *    MatMultUnit include a systolic array to calculate the multiplication of two matrix
 *                                                                                                                                                                
...
case class MatMultUnit(cfg: MatMultUnit_Config) extends Component {
  ...
  for(in_MatA_Bus_index <- 0 until cfg.in_MatA_Bus_Width)
  {
    var Bus_delay=in_MatA_Bus_index//比如说0的就不用延迟，1号线要延迟一拍，二号线要延迟两拍
    for(reg_idx <- 0 until cfg.in_Length_Max)
    {
      //例化寄存器作延时
    }
    io.in_MatA_Bus.payload.data(in_MatA_Bus_index)
  }
...
object MatMultUnit_Verilog extends App{
  val testLength=32
  val cfg = MatMultUnit_Config(testLength)
    ...
    ).generateVerilog(new MatMultUnit(cfg)).printPruned()
    
    tools.HDElkDiagramGen(SpinalVerilog(new MatMultUnit(cfg)))
}
...
object MatMultUnit_Sim extends App {
  ...
  val testLength=32
  val cfg = MatMultUnit_Config(testLength)
  val dut = SimConfig.withFstWave.compile(new MatMultUnit(cfg))
